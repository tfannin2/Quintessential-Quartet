...
while True:
    ...
    currentItem = inventory.checkInventory()
    print(currentItem)
    if currentItem:
        if currentItem == "hat":
            hat_im = pygame.image.load("Resources/baseballcap.png")
            hat_im = pygame.transform.scale(hat_im, (133, 82))
            screen.blit(hat_im, [113, 117])
    ...
    for ev in pygame.event.get():
        ...
        if ev.type == pygame.MOUSEBUTTONDOWN:
            ...
            if dailyQuoteButton[0] <= mouse[0] <= dailyQuoteButton[1] and dailyQuoteButton[2] <= mouse[1] <= dailyQuoteButton[3]:
                ...
                screen.fill((255, 255, 255))
                pygame.draw.rect(screen, (173, 211, 173), [
                                 10, (screen.get_height()/2 - 30), 355, 80])
